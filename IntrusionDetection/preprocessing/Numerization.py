...
def preHandel_data():
    ...
    with open(source_file, 'r') as data_source:
        ...
        for row in csv_reader:
            ...
            temp_line[1] = handleProtocol(row)  # protocol_type 将源文件行中3种协议类型转换成数字标识
            temp_line[2] = handleService(row)   # service 将源文件行中70种网络服务类型转换成数字标识
            temp_line[3] = handleFlag(row)      # flag 将源文件行中11种网络连接状态转换成数字标识
            temp_line[41] = handleLabel(row)    # label 将源文件行中23种攻击类型转换成数字标识
