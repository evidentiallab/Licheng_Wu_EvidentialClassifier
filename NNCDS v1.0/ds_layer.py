...
class NNCDS(object):
	...
	def _getPrototypesGradient(self, feature, label):
		...
		for i in range(self.n_prototypes):
			protograd[i] = -1 * (feature - self.prototypes[i]) * 2. * self.gamma[i] * self._prototypeActivations[i] * self._sgrad[i]
	...
	def fit(self, features, labels, max_iterations=10000, epsilon=0.08, learning_rate=0.1, momentum=0.9):
