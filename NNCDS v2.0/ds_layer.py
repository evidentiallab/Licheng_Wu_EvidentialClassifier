...
class NNCDS(object):
	...
	def _initializeExplicitParametersRandomly(self):
		...
		self._initializePrototypes(self.features, self.n_prototypes)
	...
	def _initializePrototypes(self, features, n_prototypes):
		""" prototypes """
		model = KMeans(n_clusters=n_prototypes)
		model.fit(features)
		centroids = model.cluster_centers_
		# print('centroids' + str(centroids[..., 2:]))
	...
	def _getBatchGradient(self, features, labels, nu):
		gradient = np.zeros(self.n_prototypes*(2+self.n_labels))  # each input feature introduces 6*5 parameters to be updated
	...
	def fit(self, features, labels, max_iterations=10000, epsilon=0.01, learning_rate=0.1, momentum=0.9):
		...
		prevGrad = np.zeros(self.n_prototypes*(2+self.n_labels))
